...
def format_conversation_history(history):
    ...
    formatted_history = [{"role": "system", "content": AGENT_CONTEXT}]
...
def generate_commentary_with_api(frames_base64, conversation_history):
    ...
    try:
        ...
        with requests.post(
            ...
            for chunk in response.iter_content(chunk_size=None, decode_unicode=True):
                if chunk:
                    ...
                    if not first_word_returned and words:
                        print(
                            f"Time to first word: {time.time() - start_time:.2f} seconds"
                        )
                        first_word_returned = True

                    # If we have 8+ words or end with punctuation, consider it complete
                    if any(
                        current_sentence.rstrip().endswith(p) for p in [".", "!", "?"]
                    ):
                        trimmed = current_sentence.strip()

                        # Filter out duplicates
                        if trimmed.lower() not in (
                            sent.lower() for sent in recent_sentences
                        ):
                            yield trimmed
            ...
            trimmed = current_sentence.strip()
            if trimmed and trimmed.lower() not in (
                sent.lower() for sent in recent_sentences
            ):
                yield trimmed
...
async def process_frames(frames):
    ...
    commentary_generator = generate_commentary_with_api(
        encoded_frames, format_conversation_history(conversation_history)
    )
    timestamp = time.time()
