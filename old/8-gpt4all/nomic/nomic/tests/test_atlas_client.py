...
def test_date_metadata():
    ...
    data = [
        {
            "my_date": datetime.datetime(2022, 1, i),
            "my_random_date": gen_random_datetime(),
        }
...
def test_map_embedding_progressive():
    ...
    embeddings = np.random.rand(num_embeddings, 10) + np.ones(
        shape=(num_embeddings, 10)
    )
    data = [
...
def test_interactive_workflow():
    ...
    with p.wait_for_project_lock():
        p.add_text(
            data=[{"text": random.choice(words), "id": i} for i in range(100, 200)]
        )
        p.create_index(name="UNITTEST1", indexed_field="text", build_topic_model=True)
...
def test_weird_inputs():
    ...
    p.create_index(
        name="test_weird_inputs", indexed_field="text", build_topic_model=True
    )
    with p.wait_for_project_lock():
        assert True
