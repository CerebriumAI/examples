...
class AtlasEmbeddingExplorer(Callback):
    ...
    def _create_map(self):
        seconds_since_last_build = int(
            (datetime.now() - self.last_rebuild_timestamp).total_seconds()
        )
        ...
        assert (
            set(lengths) != 1
        ), "Error in logging metadata, it is not all the same length"
        ...
        if self.max_points > 0:
            ...
            for key in self.atlas.metadata:
                ...
                if len(set(self.atlas.metadata[key])) <= 10:
                    self.atlas.metadata[key] = [
                        str(x) for x in self.atlas.metadata[key]
                    ]
        ...
        metadata = [
            dict(zip(keys, vals))
            for vals in zip(*(self.atlas.metadata[k] for k in keys))
        ]
