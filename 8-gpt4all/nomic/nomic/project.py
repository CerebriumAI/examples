...
class AtlasClass(object):
    ...
    def _validate_map_data_inputs(self, colorable_fields, id_field, data):
        ...
        for field in colorable_fields:
            if field not in data[0]:
                raise Exception(
                    f"Cannot color by field `{field}` as it is not present in the meta-data."
                )
    ...
    def _get_current_users_main_organization(self):
        ...
        **Returns:** The ID of the current users default organization
        ...
        for organization in user["organizations"]:
            if (
                organization["user_id"] == user["sub"]
                and organization["access_role"] == "OWNER"
            ):
                return organization
    ...
    def _get_project_by_id(self, project_id: str):
        ...
        if response.status_code != 200:
            raise Exception(
                f"Could not access project with id {project_id}: {response.text}"
            )
    ...
    def _validate_and_correct_arrow_upload(
        ...
        if (
            ...
            and not ATLAS_DEFAULT_ID_FIELD in data.column_names
        ):
            data = data.append_column(
                ATLAS_DEFAULT_ID_FIELD,
                pa.array([str(uuid.uuid4()) for _ in range(len(data))]),
            )
        ...
        for field in project.schema:
            ...
            if pa.types.is_string(field.type):
                ...
                if data[field.name].null_count > 0:
                    ...
                    reformatted[field.name] = pc.fill_null(
                        reformatted[field.name], "null"
                    )
                if pc.any(pc.equal(pc.binary_length(reformatted[field.name]), 0)):
                    mask = pc.equal(
                        pc.binary_length(reformatted[field.name]), 0
                    ).combine_chunks()
                    assert pa.types.is_boolean(mask.type)
                    reformatted[field.name] = pc.replace_with_mask(
                        reformatted[field.name], mask, "null"
                    )
        for field in data.schema:
            if not field.name in reformatted:
                if field.name == "_embeddings":
                    ...
                    logger.warning(
                        f"Field {field.name} present in data, but not found in table schema. Ignoring."
                    )
        ...
        ), f"Upload does not contain your specified id_field"

        if not pa.types.is_string(data[project.id_field].type):
            logger.warning(
                f"id_field is not a string. Converting to string from {data[project.id_field].type}"
            )
        ...
        if pc.max(pc.utf8_length(data[project.id_field])).as_py() > 36:
            first_match = data.filter(
                data[project.id_field].utf8_length() > 36
            ).to_pylist()[0]
            raise ValueError(
    ...
    def _get_organization(
        self, organization_name=None, organization_id=None
    ) -> Tuple[str, str]:
        """
        ...
        if organization_name is None:
            ...
            ):  # default to current users organization (the one with their name)
                organization = self._get_current_users_main_organization()
                organization_name = organization["nickname"]
                organization_id = organization["organization_id"]
            else:
                raise NotImplementedError(
                    "Getting organization by a specific ID is not yet implemented."
                )

        else:
            organization_id_request = requests.get(
                self.atlas_api_path + f"/v1/organization/search/{organization_name}",
                headers=self.header,
            )
    ...
    def _get_existing_project_by_name(self, project_name, organization_name) -> Dict:
        ...
        organization_name, organization_id = self._get_organization(
...
class AtlasProjection:
    ...
    def __init__(
        self, project: "AtlasProject", atlas_index_id: str, projection_id: str, name
    ):
        """
    ...
    def _status(self):
        response = requests.get(
            ...
            + f"/v1/project/index/job/progress/{self.atlas_index_id}",
    ...
    def download_embeddings(self, save_directory: str, num_workers: int = 10) -> bool:
        ...
        if self.project.is_locked:
            raise Exception(
                "Project is locked! Please wait until the project is unlocked to download embeddings"
            )
        ...
        def download_shard(offset, check_access=False):
            response = requests.get(
                self.project.atlas_api_path
                + f"/v1/project/data/get/embedding/{self.project.id}/{self.atlas_index_id}/{offset}/{limit}",
                headers=self.project.header,
            )
            ...
            try:
                ...
                shard_name = "{}_{}_{}.pkl".format(
                    self.atlas_index_id, offset, offset + limit
                )
                ...
                logger.error(
                    "Shard {} download failed with error: {}".format(shard_name, e)
                )
    ...
    def get_embedding_iterator(self) -> Iterable[Tuple[str, str]]:
        ...
        while True:
            response = requests.get(
                self.atlas_api_path
                + f"/v1/project/data/get/embedding/{self.id}/{self.atlas_index_id}/{offset}/{limit}",
                headers=self.header,
            )
            if response.status_code != 200:
                raise Exception(response.text)
    ...
    def vector_search(
        self, queries: np.array = None, ids: List[str] = None, k: int = 5
    ) -> Dict[str, List]:
        """
        ...
        if queries is None and ids is None:
            raise ValueError(
                "You must specify either a list of datum `ids` or numpy array of `queries` but not both."
            )
        ...
        if k > max_k:
            raise Exception(
                f"Cannot query for more than {max_k} nearest neighbors. Set `k` to {max_k} or lower"
            )
        ...
        if queries is not None:
            ...
            if queries.shape[0] > max_queries:
                raise Exception(
                    f"Max vectors per query is {max_queries}. You sent {queries.shape[0]}."
                )
        ...
        if queries is not None:
            ...
            response = requests.post(
                ...
                headers=self.project.header,
                json={"atlas_index_id": self.atlas_index_id, "datum_ids": ids, "k": k},
            )

        if response.status_code == 500:
            raise Exception(
                "Cannot perform vector search on your map at this time. Try again later."
            )
    ...
    def get_topic_data(self) -> List:
        ...
        response = requests.get(
            ...
            + "/v1/project/{}/index/projection/{}".format(
                self.project.meta["id"], self.projection_id
            ),
            headers=self.project.header,
        )
    ...
    def get_topic_density(self, time_field: str, start: datetime, end: datetime):
        ...
        response = requests.post(
            self.project.atlas_api_path
            + "/v1/project/{}/topic_density".format(self.atlas_index_id),
            headers=self.project.header,
            json={
    ...
    def vector_search_topics(
        self, queries: np.array, k: int = 32, depth: int = 3
    ) -> Dict:
        """
    ...
    def tag(self, ids: List[str], tags: List[str]):
        ...
        response = requests.post(
            self.project.atlas_api_path + "/v1/project/tag/add",
            headers=headers,
            data=payload,
        )
        if response.status_code != 200:
    ...
    def remove_tags(
        ...
        headers["Content-Type"] = "application/octet-stream"
        response = requests.post(
            self.project.atlas_api_path + "/v1/project/tag/delete",
            headers=headers,
            data=payload,
        )
        if response.status_code != 200:
...
class AtlasProject(AtlasClass):
    def __init__(
        ...
        * **add_datums_if_exists** - If specifying an existing project and you want to add data to it, set this to true.
        ...
        """
        assert (
            name is not None or project_id is not None
        ), "You must pass a name or project_id"
        ...
        results = self._get_existing_project_by_name(
            project_name=name, organization_name=organization_name
        )
        ...
        if "project_id" in results:  # project already exists
            ...
            if reset_project_if_exists:  # reset the project
                ...
                not add_datums_if_exists
            ):  # prevent adding datums to existing project explicitly
                raise ValueError(
        ...
        if project_id is None:  # if there is no existing project, make a new one.
            if unique_id_field is None:
                ...
                raise ValueError(
                    "You must specify a unique_id_field when creating a new project."
                )

            if modality is None:
                raise ValueError(
                    "You must specify a modality when creating a new project."
                )

            assert modality in [
                "text",
                "embedding",
            ], "Modality must be either `text` or `embedding`"
    ...
    def delete(self):
        ...
        logger.info(
            f"Deleting project `{self.name}` from organization `{organization_name}`"
        )
    ...
    def _create_project(
        ...
        **Returns:** project_id on success.
        ...
        logger.info(
            f"Creating project `{project_name}` in organization `{organization_name}`"
        )
    ...
    def schema(self) -> Optional[pa.Schema]:
        ...
        if "schema" in self.meta and self.meta["schema"] is not None:
            self._schema: pa.Schema = ipc.read_schema(
                io.BytesIO(base64.b64decode(self.meta["schema"]))
            )
            return self._schema
        return None
    ...
    def get_map(
        self, name: str = None, atlas_index_id: str = None, projection_id: str = None
    ) -> AtlasProjection:
        """
        Retrieves a Map
            ...
            for index in indices:
                if index.id == atlas_index_id:
                    if len(index.projections) == 0:
                        raise ValueError(
                            f"No map found under index with atlas_index_id='{atlas_index_id}'"
                        )
                    return index.projections[0]
            raise ValueError(
                f"Could not find a map with atlas_index_id='{atlas_index_id}'"
            )
            ...
            for index in indices:
                for projection in index.projections:
                    if projection.id == projection_id:
                        return projection
            raise ValueError(
                f"Could not find a map with projection_id='{atlas_index_id}'"
            )
    ...
    def create_index(
        ...
        if self.modality == "embedding":
            build_template = {
                ...
                "nearest_neighbor_index_hyperparameters": json.dumps(
                    {"space": "l2", "ef_construction": 100, "M": 16}
                ),
            ...
            if indexed_field is None:
                raise Exception(
                    "You did not specify a field to index. Specify an 'indexed_field'."
                )

            if indexed_field not in self.project_fields:
                raise Exception(
                    f"Indexing on {indexed_field} not allowed. Valid options are: {self.project_fields}"
                )
        ...
        if response.status_code != 200:
            logger.info(
                "Create project failed with code: {}".format(response.status_code)
            )
        ...
        logger.info(
            f"Created map `{projection.name}` in project `{self.name}`: {projection.map_link}"
        )
        return projection
    ...
    def get_data(self, ids: List[str]) -> List[Dict]:
        ...
        if isinstance(ids[0], list):
            raise ValueError(
                "You must specify a list of ids when getting data, not a nested list."
            )
        response = requests.post(
    ...
    def add_text(
        ...
        if DataFrame is not None and isinstance(data, DataFrame):
            ...
            raise ValueError(
                "Data must be a pandas DataFrame, list of dictionaries, or a pyarrow Table."
            )
    ...
    def add_embeddings(
        ...
        if shard_size is not None:
            raise DeprecationWarning(
                "shard_size is deprecated and no longer has any effect"
            )
        if num_workers is not None:
            raise DeprecationWarning(
                "num_workers is deprecated and no longer has any effect"
            )
        ...
        assert (
            len(data) == embeddings.shape[0]
        ), "Data and embeddings must have the same number of rows."
        ...
        pyarrow_embeddings = pa.FixedSizeListArray.from_arrays(
        ...
        def send_request(i):
            ...
            with io.BytesIO() as buffer:
                ...
                feather.write_feather(
                    data_shard, buffer, compression="zstd", compression_level=6
                )
                buffer.seek(0)
                ...
                for future in done:
                    ...
                    if response.status_code != 200:
                        try:
                            ...
                            if (
                                "more datums exceeds your organization limit"
                                in response.json()
                            ):
                                return False
                            ...
                            if response.status_code == 413:
                                ...
                                logger.error(
                                    "Shard upload failed: you are sending meta-data that is too large."
                                )
                                pbar.update(1)
                                response.close()
                                failed += shard_size
                                ...
                                failure_fraction = errors_504 / (
                                    failed + succeeded + errors_504
                                )
                                if (
                                    failure_fraction > 0.5
                                    and errors_504 > shard_size * 3
                                ):
                                    raise RuntimeError(
                                ...
                                logger.error(
                                    f"{self.name}: Shard upload failed: {response}"
                                )
                                failed += shard_size
                                pbar.update(1)
                                response.close()
