...
class GPT4AllGPU:
    def __init__(self, llama_path=None):
        ...
        print(
            f"Mem needed: {self.model.get_memory_footprint() / 1024 / 1024 / 1024:.2f} GB"
        )
...
class GPT4All:
    def __init__(
        self, model="gpt4all-lora-quantized", force_download=False, decoder_config=None
    ):
        """
        ...
        if force_download or not (
            self.model_path.exists() and self.model_path.stat().st_size > 0
        ):
    ...
    def open(self):
        ...
        self.bot = subprocess.Popen(
            creation_args, stdin=subprocess.PIPE, stdout=subprocess.PIPE
        )
    ...
    def _download_model(self):
        ...
        if not self.model_path.exists():
            response = requests.get(
                f"https://the-eye.eu/public/AI/models/nomic-ai/gpt4all/{self.model}.bin",
                stream=True,
            )
            if response.status_code == 200:
